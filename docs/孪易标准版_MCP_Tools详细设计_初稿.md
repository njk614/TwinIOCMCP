...
一个孪生体实例必须来自某个孪生体类别。孪生体类别可以绑定台账表、时序表、事件表字段。

| Tool | 能力 | 主要参数 |
| --- | --- | --- |
| `create_twin_category_folder` | 创建孪生体类别文件夹 | `parent_id?`、`name` |
| `list_twin_category_folders` | 查询类别文件夹 | `parent_id?` |
| `create_twin_category` | 创建孪生体类别 | `folder_id?`、`name`、`model_config?` |
| `list_twin_categories` | 查询孪生体类别 | `keyword?`、`folder_id?` |
| `get_twin_category` | 查询类别详情 | `category_id` |
| `update_twin_category` | 编辑孪生体类别 | `category_id`、`patch`、`mode`、`confirm` |
| `delete_twin_category` | 删除孪生体类别 | `category_id`、`mode`、`confirm` |
| `copy_twin_category` | 复制孪生体类别 | `source_category_id`、`target_name` |
| `move_twin_category` | 移动孪生体类别 | `category_id`、`target_folder_id` |
...
| Tool | 能力 | 主要参数 |
| --- | --- | --- |
| `create_twin_instance` | 创建孪生体实例 | `scene_id`、`category_id`、`name`、`position?`、`resource_ref?` |
| `list_twin_instances` | 查询实例列表 | `scene_id`、`category_id?`、`keyword?` |
...
| `list_twin_category_folders` | 查询孪生体类别文件夹 | `operational_data`、`parent_id?`、`tree?` | `items` 或 `tree` | 根：`GET /v1/{operationalData}/{assetFolderType}/assetFolder/root`；子级：`GET /v1/{operationalData}/{assetFolderType}/assetFolder/{parentFolderId}` |
| `create_twin_category` | 创建租户级孪生体类别 | `operational_data`、`folder_id`、`name`、`description?`、`model_config?`、`mode`、`confirm` | `category` | `POST /v1/{operationalData}/twinCategory/Save` |
| `list_twin_categories` | 查询租户级孪生体类别列表 | `operational_data`、`folder_id`、`keyword?`、`page?`、`page_size?` | `items`、`total?` | `POST /v1/{operationalData}/twinCategory/folder/{folderId}` |
...
| `delete_twin_category` | 删除孪生体类别 | `operational_data`、`category_ids`、`mode`、`confirm` | `delete_result`、`dependency_summary` | `POST /v1/{operationalData}/twinCategory/delete` |
| `copy_twin_category` | 复制孪生体类别 | `source_category_id`、`target_name?`、`target_folder_id?`、`mode`、`confirm` | `source_category_id`、`target_category` | `POST /v1/twinCategorys/copy`；行业到租户：`POST /v1/copyIndustryData/twinCategory/{twinCategoryId}/folder/{folderId}` |
| `move_twin_category` | 移动孪生体类别到指定文件夹 | `operational_data`、`category_ids`、`target_folder_id`、`mode`、`confirm` | `category` | `POST /v1/{operationalData}/twinCategory/move/folder/{folderId}` |
...
| `list_twin_ledger_fields` | 查询孪生体台账字段 | `operational_data`、`category_id` 或 `scene_id+twin_category_config_id` | `items` | 租户级：`GET /v1/{operationalData}/twinCategory/{twinCategoryId}`；场景级：`GET /v1/{operationalData}/location/{locationId}/twinCategory/{twinCategoryConfigID}/field` |
...
| Tool | 中文说明 | MCP 入参 | MCP 出参 `data` | 后端接口映射（待确认） |
| --- | --- | --- | --- | --- |
| `create_twin_instance` | 创建场景中的孪生体实例 | `operational_data`、`scene_id`、`twin_category_config_id`、`name`、`hierarchy_id?`、`region_id?`、`ledger_data?`、`position?`、`mode`、`confirm` | `instance` | `POST /v1/{operationalData}/location/{locationId}/twinCategory/{twinCategoryConfigID}/data/add`；批量用 `/data/batchAdd` |
| `list_twin_instances` | 查询场景孪生体实例列表 | `operational_data`、`scene_id`、`twin_category_config_id?`、`hierarchy_id?`、`keyword?`、`page?`、`page_size?` | `items`、`total` | 类别内：`POST /v1/{operationalData}/location/{locationId}/twinCategory/{twinCategoryConfigID}/data`；全场景：`POST /v1/{operationalData}/location/{locationId}/data` 或 `/twinCategoryData` |
| `get_twin_instance` | 查询单个孪生体实例详情 | `operational_data`、`scene_id`、`twin_category_config_id`、`instance_id` | `instance`、`category`、`resource_refs`、`data_summary` | 列表接口后由 MCP 按 `twinCategoryDataID` 或实例编号过滤 |
| `update_twin_instance` | 修改孪生体实例属性或位置 | `operational_data`、`scene_id`、`twin_category_config_id`、`instance_id`、`patch`、`mode`、`confirm` | `instance`、`diff?` | `POST /v1/{operationalData}/location/{locationId}/twinCategory/{twinCategoryConfigID}/data/batchEdit` |
| `delete_twin_instance` | 删除孪生体实例 | `operational_data`、`scene_id`、`twin_category_config_id`、`instance_ids`、`mode`、`confirm` | `delete_result`、`dependency_summary` | `POST /v1/{operationalData}/location/{locationId}/twinCategory/{twinCategoryConfigID}/data/delete`；清空用 `/data/deleteAll` |
...
| Tool | 中文说明 | MCP 入参 | MCP 出参 `data` | 后端接口映射（待确认） |
| --- | --- | --- | --- | --- |
| `upsert_twin_instance_ledger_data` | 新增或更新实例台账数据 | `operational_data`、`scene_id`、`twin_category_config_id`、`instance_id`、`data`、`mode`、`confirm` | `ledger_data`、`diff?` | 新增：`POST /v1/{operationalData}/location/{locationId}/twinCategory/{twinCategoryConfigID}/data/add`；更新：`/data/batchEdit` |
| `list_twin_instance_ledger_data` | 查询实例台账数据 | `operational_data`、`scene_id`、`twin_category_config_id`、`fields?` | `items` | `POST /v1/{operationalData}/location/{locationId}/twinCategory/{twinCategoryConfigID}/data` |
| `delete_twin_instance_ledger_data` | 删除实例台账数据 | `operational_data`、`scene_id`、`twin_category_config_id`、`instance_ids?`、`mode`、`confirm` | `delete_result` | `POST /v1/{operationalData}/location/{locationId}/twinCategory/{twinCategoryConfigID}/data/delete` |
...
| 孪生体类别 | 已定位 | 租户级类别使用 `twinCategory` + `assetFolderType=TwinCategory` |
...
| 孪生体类别 | `list_twin_categories`、`create_twin_category_folder`、`list_twin_category_folders`、`get_twin_category`、`create_twin_category`、`update_twin_category`、`delete_twin_category` |
...
| V1.1 | 下个节点 | P0 对象完整 | 场景、层级、点线面、类别、实例完整 CRUD |
...
list_twin_categories / create_twin_category
